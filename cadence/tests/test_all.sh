...
assert_output_contains "Query PYUSD balance > 0" "Result" \
    $FLOW scripts execute cadence/scripts/get_pyusd_balance.cdc 0xf8d6e0586b0a20c7 -n emulator
...
assert_success "Create raffle with valid params" \
...
assert_output_contains "Raffle exists on chain" "Result" \
...
assert_success "Deposit $100 to raffle from test-account" \
    $FLOW transactions send cadence/transactions/deposit.cdc 1 100.0 \
    --signer "$TEST_ACCOUNT" -n emulator

assert_output_contains "Deposit recorded on chain" "Result" \
    $FLOW scripts execute cadence/scripts/get_deposit.cdc 1 0x179b6b1cb6755e31 -n emulator
...
assert_success "Harvest yield for raffle" \
...
# First create a new raffle + deposit for withdrawal test
assert_success "Create raffle for withdrawal test" \
    $FLOW transactions send cadence/transactions/create_raffle.cdc \
    "Withdraw Test House" "For testing withdrawal" 50000.0 \
    --signer "$EMULATOR_ACCOUNT" -n emulator

assert_success "Deposit to withdrawal test raffle" \
    $FLOW transactions send cadence/transactions/deposit.cdc 2 200.0 \
    --signer "$TEST_ACCOUNT" -n emulator

assert_success "Withdraw principal from raffle" \
    $FLOW transactions send cadence/transactions/withdraw.cdc 2 \
    --signer "$TEST_ACCOUNT" -n emulator

assert_fail "Withdraw from raffle with no deposit should fail" \
    $FLOW transactions send cadence/transactions/withdraw.cdc 2 \
    --signer "$TEST_ACCOUNT" -n emulator
...
assert_output_contains "Get all deposits for raffle" "Result" \
